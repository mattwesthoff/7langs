...
mysort([X], [X]).
mysort([Head|Rest], Output) :- sort(Rest, [Next|NextRest]), Next > Head, append([Head], [Next], Output).
mysort([Head|Rest], Output) :- sort(Rest, [Next|NextRest]), Next =< Head, append([Next], [Head], Output).